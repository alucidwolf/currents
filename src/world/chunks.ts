...
export class ChunkManager {
  ...
  private readonly pending: Array<{ cx: number; cz: number }> = [];
  ...
  update(position: THREE.Vector3, immediate = false): void {
    ...
    const budget = immediate ? this.pending.length : BUILD_BUDGET_PER_FRAME;
    for (let i = 0; i < budget && this.pending.length > 0; i++) {
      const next = this.pending.shift()!;
      // The player may have moved on before this got built.
      if (!this.chunks.has(chunkKey(next.cx, next.cz))) {
        this.build(next.cx, next.cz);
      }
    }
  }
  ...
  private reconcile(cx: number, cz: number): void {
    ...
    for (const item of wanted) this.pending.push({ cx: item.cx, cz: item.cz });
  ...
  private build(cx: number, cz: number): void {
    ...
    // Chunks are always in front of the fog wall when visible; skipping the
    // per-frame sphere test is free performance at this scale.
    ...
    const built = buildChunkDecor(this.worldSeed, cx, cz, this.terrain);
    let decor: THREE.Mesh | null = null;

    if (built) {
      decor = new THREE.Mesh(built.geometry, this.decorMaterial);
